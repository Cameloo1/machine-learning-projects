...
def create(cfg: Dict[str, Any]) -> Any:
    ...
    if name == "equal_weight":
        return EqualWeightStrategy(cfg)
    if name == "vol_target_trend":
        ...
        base_cfg = {"name": base_name, "params": base_params}
        base_strategy = create(base_cfg)
        return VolTargetOverlay(base_strategy=base_strategy, params=params, base_cfg=base_cfg)
    if name == "ml_gated":
