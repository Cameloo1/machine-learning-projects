#!/usr/bin/env python
"""
Download and cache daily OHLCV data with validation and fallbacks.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download daily market data.")
    parser.add_argument(
        "--tickers",
        required=True,
...
def setup_logger(level: str) -> logging.Logger:
    ...
    return logging.getLogger("download_market_data")


def build_http_session(logger: logging.Logger) -> Optional["requests.Session"]:
    if requests is None:
...
def _extract_yahoo_actions(result: Dict[str, Any]) -> Optional[pd.DataFrame]:
    ...
    for ts_key, info in dividends.items():
        try:
            ts = pd.to_datetime(int(ts_key), unit="s").normalize()
        except Exception:
            ts = pd.to_datetime(info.get("date"), unit="s", errors="coerce")
        rows.append(
            {
                "ts": ts,
                "type": "dividend",
    ...
    for ts_key, info in splits.items():
        try:
            ts = pd.to_datetime(int(ts_key), unit="s").normalize()
        except Exception:
            ts = pd.to_datetime(info.get("date"), unit="s", errors="coerce")
        rows.append(
            {
                "ts": ts,
                "type": "split",
                "value": info.get("splitRatio"),
                "numerator": info.get("numerator"),
                "denominator": info.get("denominator"),
            }
        )
...
def fetch_yahoo_chart_direct(
    ...
    payload = retry_call(
        _fetch, logger, f"yahoo-chart:{symbol}", tries=4, base_delay=2.0
    )
    ...
    actions_df = _extract_yahoo_actions(res)
    return df, actions_df
...
def download_yahoo(
    ...
    if yf is None:
        raw_df, actions_df = fetch_yahoo_chart_direct(
            symbol, start, end, session, logger
        )
    ...
    try:
        ...
        logger.warning(
            "yfinance failed, attempting direct chart fallback ticker=%s err=%s",
            symbol,
            exc,
        )
        raw_df, actions_df = fetch_yahoo_chart_direct(
    ...
    if raw_df is None or raw_df.empty:
        logger.warning(
            "yfinance returned empty frame, using direct chart fallback ticker=%s",
            symbol,
        )
        raw_df, actions_df = fetch_yahoo_chart_direct(
            symbol, start, end, session, logger
    ...
    if not required_cols.issubset(set(raw_df.columns)):
        logger.warning(
            "yfinance missing columns, using direct chart fallback ticker=%s",
            symbol,
        )
        raw_df, actions_df = fetch_yahoo_chart_direct(
            symbol, start, end, session, logger
    ...
    if raw_df[["Open", "High", "Low", "Close"]].dropna(how="all").empty:
        logger.warning(
            "yfinance data all NaN, using direct chart fallback ticker=%s",
            symbol,
        )
        raw_df, actions_df = fetch_yahoo_chart_direct(
            symbol, start, end, session, logger
...
def main() -> int:
    ...
    ensure_dirs()
    session = build_http_session(logger)
